...
function() {
	...
	function namespace(name) {
		for (var arr = name.split(","), r = 0, len = arr.length; r < len; r++) for (var i = 0, k, n = arr[r].split("."), parent = {}; k = n[i]; i++) 0 === i ? eval("(typeof " + k + ')==="undefined"?(' + k + '={}):"";parent=' + k) : parent = parent[k] = parent[k] || {}
	}
	function isEmbedded() {
	...
	function jsonExtend(a, b, c) {
		var d;
		if (b instanceof Array) for (d = 0, len = b.length; d < len; d++) jsonExtend(a, b[d], c);
	...
	function getBrowserInfo() {
		...
		for (e = [
			...
			["IE", /msie[ ](\d+\.\d+)/],
			["Safari", /safari\/([\d\.]+)/]
		], a = 0; a < e.length; a++) if (b = d.match(e[a][1])) {
	...
	function getCommonData(a) {
		var b, c = {},
			d = a.logType;
		for (c.pin_sid = getParameter("sid") || getCookie("sid") || "", c.report_ts = getCurTime(), c.scr = lr.resolution, c.token = lr.hex_md5(c.report_ts + lr.md5Key), c.ut = "s", c.clt = "web", c.jvr = lr.jvr || "3.0.5", c.std = lr.siteId, c.tpc = (a.topic || lr.topic) + "." + (d === lr.logType.pv ? "pv" : d === lr.logType.cl ? "cl" : "ot"), c.uuid = lr.uuid, b = 0; b < lr.getCommonDataF.length; b++) lr.getCommonDataF[b](a, c);
		return c
	}
	...
	lr.getCommonDataF = [], lr.updateCookieF = [], lr.initBF = [], lr.initAF = [], lr.getPrivateDataF = [], lr.clickCBF = [], lr.logOldF = [], lr.getCommonDataF.push(function(a, b) {
		...
		if ("undefined" != typeof jaq && jaq instanceof Array) for (a = 0; a < jaq.length; a++)"account" === jaq[a][0] && (lr.siteId = jaq[a][1]);
		for (function() {
			var a = "";
			if (document.currentScript) a = document.currentScript.src;
			else if (document.scripts) {
				var b;
				for (b = 0; b < document.scripts.length; b++) if ("interactive" === document.scripts[b].readyState) {
		...
		if (s) {
			...
			q.push(s || m), q.push(v || n), q.push(w || o), q.push(x || p), p = q[3], t = !0
		} else {
			...
			if (z && z.indexOf(lr.ckDomain) < 0) {
				for (y = lr.seo, e = 0; e < y.length; e++) {
					...
					if (z.indexOf(B[0].toLowerCase()) > -1 && lr.refUrl.indexOf((B[1] + "=").toLowerCase()) > -1) {
						...
						q.push(B[0]), q.push("-"), q.push("organic"), q.push(C || "not set"), p = q[3], A = !0;
						break
					}
				}
		...
		if (!(a.logType !== lr.logType.pd && a.logType !== lr.logType.hm && a.logType !== lr.logType.cl && a.logType !== lr.logType.pv || a.logType === lr.logType.cl && a.clickType !== lr.clickType.Clstag)) {
			var d = {
				...
				i = "",
				j = [];
			i = a.topic ? a.topic : a.logType === lr.logType.pv ? "m.100000" : "other.000000", lr.utmp ? (f = 0 === lr.utmp.indexOf("http:") || 0 === lr.utmp.indexOf("https:") ? lr.utmp : document.location.protocol + "//" + document.domain + lr.utmp, f += (f.indexOf("?") >= 0 ? "&jav=" : "?jav=") + h) : f = lr.curUrl, a.logType === lr.logType.cl ? (j = a.cls.split("|"), a.pdsvj = {
				t1: j[0],
				t2: j[1],
				cb: lr.sequenceNum
	...
	var getAbsoluteXPath = function(a) {
		...
		isReportElement = function(a) {
			...
			if (a && a.tagName) {
				...
				b = !("html" === c || "body" === c || a.id && "tol_selected_xelemts_area" === a.id || a.parentElement.id && "tol_selected_xelemts_area" === a.parentElement.id) && ("a" === c || isLeafElemt(a))
			}
			return b
		};
	...
	var coordinateInfo = function(a) {
		...
		xpathReport = function(a) {
			try {
				...
				if (isReportElement(b)) {
					var c = coordinateInfo(a);
					if (!c || c.x <= 0 || c.y <= 0) return;
					var d = getXPath(b) || "",
						e = b.parentNode ? getXPath(b.parentNode) : "",
						f = b.parentNode ? getAbsoluteXPath(b.parentNode) + "/" : "",
						g = getAbsoluteXPath(b) || "",
						h = g ? g.split("/").length : 1,
						i = document.body.clientWidth,
						j = i > 0 ? (c.x / i).toFixed(4) : 0,
						k = c.scrollHeight > 0 ? (c.y / c.scrollHeight).toFixed(4) : 0;
					magictreelog("magictree", "Z", encodeURIComponent(d), encodeURIComponent(e), encodeURIComponent(f), h, c.x + "x" + c.y, c.scrollWidth + "x" + c.scrollHeight, c.offsetLeft, j, k)
				}
			} catch (a) {
		...
		loadJS = function(a) {
			...
			b.type = "application/javascript", b.src = a, b.charset = "UTF-8", document.body.appendChild(b)
		},
		loadCSS = function(a) {
	...
	if (typepar && ("query" === typepar || "apply" === typepar)) try {
		loadCSS("//magicforest.jd.com/x.css"), loadJS("//magicforest.jd.com/tol.min.js")
	} catch (a) {
	...
	window.initMagicModel = initMagicModel, document.addEventListener(lr.clickEvent, function(a) {
		...
		if (g.clientX || g.clientY || g.pageX || g.pageY) {
			...
			for (; !f && "BODY" !== d.nodeName && "HTML" !== d.nodeName && (d = d.parentNode, d && ("BODY" !== d.nodeName || "HTML" === d.nodeName));) f = d.getAttribute("clstag");
			var h;
			if (f) {
				var i = f.split("|"),
					j = i[1],
		...
		function c(a) {
			...
			for (var b, c = o ? "0123456789ABCDEF" : "0123456789abcdef", d = "", e = 0; e < a.length; e++) b = a.charCodeAt(e), d += c.charAt(b >>> 4 & 15) + c.charAt(15 & b);
			return d
		}
		function d(a) {
